export class WeeklyStatsAnalyticsRenderer {
    ...
    public renderStats(stats: { totalBalance: number, monthlyIncome: number, monthlyExpense: number }) {
        const balanceEl = document.getElementById('weekly-total-balance');
        const incomeEl = document.getElementById('weekly-monthly-income');
        const expenseEl = document.getElementById('weekly-monthly-expense');
    ...
    public renderAnalyticsSummary(data: { type: string; total: number; categories: { name: string; total: number }[] }[]) {
        const container = document.getElementById('weekly-analytics-summary');
        if (!container) return;
    ...
    public renderExpenseChart(data: { type: string; total: number; categories: { name: string; total: number }[] }[]) {
        const container = document.getElementById('weekly-analytics-chart-container');
        if (!container) return;
        ...
        relevantGroups.forEach(g => {
            ...
            let color = WeeklyStatsAnalyticsRenderer.COLORS.FALLBACK;
            if (isProfit) color = WeeklyStatsAnalyticsRenderer.COLORS.PROFIT;
            else if (isWithdrawal) color = WeeklyStatsAnalyticsRenderer.COLORS.WITHDRAWAL;
        ...
        const barsHtml = items.map(item => {
            ...
            return `
                <div style="margin-bottom: 8px;">
                    ...
                    <div style="width: 100%; background: ${WeeklyStatsAnalyticsRenderer.COLORS.BG_BAR}; height: 6px; border-radius: 3px; overflow: hidden;">
