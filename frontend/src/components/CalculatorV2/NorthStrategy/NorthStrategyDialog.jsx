 ...
 * используется (заменён на «Север GPT», см. ../NorthGptStrategy/). Код не удалён, а
 * отключён: кнопка скрыта (canShowNorthButton = false в UniversalOptionsCalculator).
 * Не «чинить», не развивать и не возвращать без явной просьбы.
 *
      ...
      if (chain && chainHasDate(chain, targetIso)) {
        ...
        try {
          ...
          const analysis = analyzeNorthStrategy({
            ...
            callStrikeMin: formParams.callStrikeMin,
            callStrikeMax: formParams.callStrikeMax,
