...
function StrikeScale({ 
  ...
  isLocked = false, // Если true — шкала заблокирована, нельзя перетаскивать флажки
}) {
  ...
  const { topFlags, bottomFlags, topYLevels, bottomYLevels } = useMemo(() => {
    ...
    groupedOptions.forEach(opt => {
      ...
      // Проверяем, зафиксирован ли опцион (для визуального отличия и блокировки перетаскивания)
      // ЗАЧЕМ: Зафиксированные опционы показываем с иконкой замка и блокируем перетаскивание
      const isLockedPosition = opt.isLockedPosition || false;
      ...
      const flag = {
        ...
        isLockedPosition, // Флаг зафиксированной позиции для визуального отличия
      };
  ...
  const handleFlagMouseDown = (e, flag) => {
    ...
    // Блокируем перетаскивание только для зафиксированных опционов (isLockedPosition)
    // ВАЖНО: Новые опционы (без isLockedPosition) можно перетаскивать даже в режиме isLocked
    // ЗАЧЕМ: Позволяет добавлять и настраивать новые опционы к зафиксированной конфигурации
    if (flag.optionIds && flag.optionIds.length > 0) {
      const firstOptionId = flag.optionIds[0];
      const option = options.find(opt => opt.id === firstOptionId);
      if (option?.isLockedPosition) {
        console.log('🔒 Опцион зафиксирован — перетаскивание заблокировано:', firstOptionId);
        return;
      }
    }
  ...
  }, [ticker, uniqueDates, selectedExpirationDate, selectedDateFilter]);
          ...
          return (
            <div
              ...
              className={`absolute pointer-events-auto select-none ${
                flag.type === "ticker" ? "cursor-default" : flag.isLockedPosition ? "cursor-not-allowed" : isDragging ? "cursor-grabbing" : "cursor-grab"
              }`}
              style={{
              ...
              title={flag.isLockedPosition ? 'Зафиксированная позиция (нельзя перетаскивать)' : 'Перетащите для изменения страйка'}
            >
              <div
                className="relative rounded-md px-2 py-1 flex items-center gap-1 shadow-md"
                style={{ backgroundColor: flag.color }}
              >
                {/* Иконка замка для зафиксированных позиций */}
                {flag.isLockedPosition && (
                  <span className="text-white text-xs mr-0.5" title="Зафиксировано">🔒</span>
                )}
        ...
        {displayBottomFlags.map((flag) => {
          ...
          return (
            <div
              key={flag.id}
              className={`absolute pointer-events-auto select-none ${
                flag.isLockedPosition ? "cursor-not-allowed" : isDragging ? "cursor-grabbing" : "cursor-grab"
              }`}
              style={{
                left: `${leftPosition}px`,
                top: `${topPosition}px`,
