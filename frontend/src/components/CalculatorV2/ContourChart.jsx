...
/**
 * ContourChart - Контурная карта P&L (Цена vs Дни до экспирации)
 */
function ContourChart({ options = [], currentPrice = 0, daysRemaining = 0 }) {
  ...
  const chartData = useMemo(() => {
    ...
    const trace = {
      ...
      contours: {
        showlabels: true,
        labelfont: { color: themeColors.text, size: 10 }
      },
    ...
    const layout = {
      title: {
        text: 'Contour Map: P&L (Price × Days)',
    ...
    return (
      <div className="h-96 flex items-center justify-center border-2 border-dashed border-border rounded-lg">
        <div className="text-center">
          <p className="text-muted-foreground mb-2">Контурная карта недоступна</p>
