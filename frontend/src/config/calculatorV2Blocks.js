/**
 * Конфигурация блоков для Options Calculator V2
 * Все блоки всегда включены и отображаются в заданном порядке
 */
...
/**
 * Получить все активные блоки (всегда все блоки включены)
 * @returns {Array} Массив всех блоков, отсортированных по order
 */
export const getActiveBlocks = () => {
  return calculatorV2Blocks.main
...
export const getBlockById = (blockId) => {
  return calculatorV2Blocks.main.find(b => b.id === blockId) || null;
};
...
/**
 * Проверить, требует ли блок выбранный тикер
 * @param {string} blockId - ID блока
 * @returns {boolean}
 */
...
/**
 * Проверить, требует ли блок позиции
 * @param {string} blockId - ID блока
 * @returns {boolean}
 */
...
export default calculatorV2Blocks;
