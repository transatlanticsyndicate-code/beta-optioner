 ...
 * Записывает результат в tvc_refresh_command (тот же ключ, но тип sendPrIV_tocallc).
 * Калькулятор должен забрать эти данные и применить к опционам в таблице.
 *
 * Возвращает pendingRefresh (данные для применения) и markProcessed (вызвать после применения).
 * ВАЖНО: Команда НЕ помечается processed пока калькулятор не вызовет markProcessed —
 * это гарантирует, что данные не потеряются если опционы ещё не загружены.
 *
...
export function useExtensionRefreshCommand(pollInterval = 1500) {
  ...
  useEffect(() => {
    const checkCommand = () => {
      try {
        ...
        // Обрабатываем только sendPrIV_tocallc, только необработанные, только новые
        if (
        ...
        // Запоминаем timestamp чтобы не обрабатывать повторно в polling
        ...
        // Сохраняем данные в state — калькулятор применит их когда опционы будут загружены
        setPendingRefresh({
          ticker: command.ticker || '',
          currentPrice: command.currentPrice || null,
          timestamp: command.timestamp,
          options: (command.options || []).map(o => ({
            // Конвертируем 'C'/'P' → 'CALL'/'PUT' для совместимости с калькулятором
  ...
  // Вызвать после успешного применения данных
  // ЗАЧЕМ: Помечает команду в localStorage как processed и очищает pendingRefresh
  const markProcessed = useCallback(() => {
    ...
    console.log('✅ [ExtRefresh] Команда помечена как обработанная');
  }, []);
