...
export function describeAnchorCalibration({ mode, calibratedVolatility, anchorVolatility, reason } = {}) {
  ...
  if (mode === 'fallback') {
    ...
    return 'Факт брокера недостижим для модели этого опциона (убыток/прибыль больше уплаченной премии — например, из-за комиссий) — P&L показан как у брокера, разница сохраняется постоянной суммой';
  }
