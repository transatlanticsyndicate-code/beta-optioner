...
export function calculateTotalPremium(options) {
  ...
  return options
    ...
    .reduce((total, option) => {
      ...
      return total + (premium * quantity * multiplier * 100);
    }, 0);
}
...
export function calculateRequiredCapital(options, currentPrice = 245.27, positions = []) {
  ...
  visibleOptions.forEach(option => {
    if (!option.strike || !option.premium) return;

    const quantity = Math.abs(option.quantity || 1);
    const strike = option.strike;
    const optionPremium = option.premium || 0;
...
function calculateSpreadMargin(spreads, currentPrice) {
  ...
  spreads.forEach(spread => {
    ...
    if (spread.type === 'vertical_call') {
      const netCredit = (spread.upper.premium || 0) - (spread.lower.premium || 0);
      const spreadMargin = Math.max(0, (width - netCredit) * quantity * 100);
      totalMargin += spreadMargin;
    } else if (spread.type === 'vertical_put') {
      const netCredit = (spread.lower.premium || 0) - (spread.upper.premium || 0);
      const spreadMargin = Math.max(0, (width - netCredit) * quantity * 100);
      totalMargin += spreadMargin;
    }
  });
 ...
 */
export function calculatePLMetrics(options, currentPrice, positions = [], daysRemaining = 0) {
  ...
  const { prices, totalPLArray } = calculatePLDataForMetrics(options, currentPrice, positions, daysRemaining);
...
export function calculateOptionPL(option, price) {
  ...
  if (!strike || !premium || !quantity) return 0;
