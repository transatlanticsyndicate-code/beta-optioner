/**
 * ⚠️ ЧАСТИЧНО УСТАРЕЛО (DEPRECATED). Сам математический анализатор «Севера» отключён
 * и НЕ используется (заменён на «Север GPT»). НО из этого файла «Север GPT»
 * переиспользует read-only константу NORTH_KINDS — её НЕ удалять. Остальной код не
 * «чинить», не развивать и не возвращать без явной просьбы.
 *
