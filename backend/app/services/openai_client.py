...
# Обе конструкции: «актив + опционы» и «только опционы».
COMBINATION_SCHEMA = {
    ...
    "schema": {
        ...
        "$defs": {"combo": _COMBO_DEF},
    },
}

# Только опционная конструкция — когда вариант с активом выключен в настройках.
OPTIONS_ONLY_SCHEMA = {
...
class OpenAIClient:
    ...
    def select_combinations(self, user_prompt, constraints, chain, with_asset=True):
        ...
        params = {
            ...
            "response_format": {
                "type": "json_schema",
                "json_schema": COMBINATION_SCHEMA if with_asset else OPTIONS_ONLY_SCHEMA,
            },
            "messages": messages,
        }
